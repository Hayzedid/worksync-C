...
export function CommentList({ comments, onAdd, onEdit, onDelete, contextId, localUser }: CommentListProps) {
  ...
  // listen for remote comments (normalize to `comments:new` event)
  useEffect(() => {
    if (!socket) return;
    ...
    socket.on('comments:new', handleRemote);
    return () => { socket.off('comments:new', handleRemote); };
  }, [socket, onAdd]);
      ...
      <form
        onSubmit={e => {
          ...
          if (input && input.value && input.value.trim()) {
            ...
            onAdd(content);
            // emit socket event for remote clients (use plural events)
            if (socket) socket.emit('comments:add', { contextId: contextId ?? undefined, content });
      ...
      {comments.map(c => (
        <div key={c.id} className="bg-white rounded shadow p-3 flex flex-col gap-1 border border-[#0CABA8]/20">
          ...
          <div className="flex gap-2 mt-1">
            ...
            <Button className="bg-[#008F8C] text-white" onClick={() => onDelete(c.id)}><Trash2 className="icon-delete" /></Button>
          </div>
        </div>
      ))}
    </div>
