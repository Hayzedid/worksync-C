...
export function TagList({ tags, onAdd, onRemove }: TagListProps) {
  return (
    <div>
      ...
      >
        <input name="tag" className="flex-1 px-3 py-2 rounded border border-[#0CABA8]/30 focus:outline-none focus:ring-2 focus:ring-[#0FC2C0] text-[#015958] bg-[#F6FFFE]" placeholder="Add a tag..." />
      ...
      <div className="flex flex-wrap gap-2">
        {tags.map(tag => (
          <span key={tag.id} className="inline-flex items-center px-3 py-1 rounded-full bg-[#0CABA8] text-white text-sm">
            {tag.label}
            <button className="ml-2" onClick={() => onRemove(tag.id)}><X className="h-4 w-4" /></button>
          </span>
        ))}
      </div>
    </div>
